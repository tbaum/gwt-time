 ...
 * @since 1.0
 * @deprecated is not useable in GWT as the get() method is dependent on
 * 				the Class.isAssignableFrom() method, which is not available in
 * 				GWT
 */
class ConverterSet {
     ...
     * @throws IllegalArgumentException if multiple converters match the type
     * equally well
     */
    ...
    private static Converter selectSlow(ConverterSet set, Class<?> type) {
        ...
        /*
         * GWT does not support the isAssignableFrom(Class) method. Therefore,
         * we cannot check for superInterfaces. Therefore, we will throw an
...
//        		if(j!=i){
//        			Class<?> jClass = converters[j].getSupportedType();
//        			if(isSubclass(supportedType,jClass)){
