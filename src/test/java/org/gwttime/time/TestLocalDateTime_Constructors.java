...
public class TestLocalDateTime_Constructors extends JodaGwtTestCase {
	...
	public void testFactory_FromDateFields() throws Exception {
    	Date date = new Date(1970-1900,2,3,4,5,6);
    	
    	//-sf- this is not the correct expected value..should be (1970,3,3,4,5,6,0), since 
    	//java.util.Date counts months from 0, not 1
        LocalDateTime expected = new LocalDateTime(1970, 3, 3, 4, 5 ,6, 0);
        assertEquals(expected, LocalDateTime.fromDateFields(date));
        try {
