...
//    }
    ...
    protected BasePeriod(ReadableDuration duration,PeriodType type,Chronology chrono){
    	super();
    	setValues(duration, type, chrono);
    }

    protected BasePeriod(ReadableInterval intervale,PeriodType type,Chronology chrono){
    	super();
    	setValues(intervale,type,chrono);
    }
    ...
    protected BasePeriod(ReadablePeriod period, PeriodType type, Chronology chrono){
    	...
    	if(this instanceof ReadWritablePeriod){
    		...
    		iValues = new MutablePeriod(period,type,chrono).getValues(); //-sf- using period.toMutablePeriod() will cause
    																	 //infinite recursion. Thus, use constructor directly
    }
    private void setValues(Object duration, PeriodType type,
    		Chronology chrono) {
    	type = checkPeriodType(type);
    	iType=type;
    	if(this instanceof ReadWritablePeriod){
			...
			for(int i=0;i<iValues.length;i++)
				((ReadWritablePeriod)this).setValue(i,iValues[i]);
    	}else{
    		if(duration instanceof ReadableDuration)
    			iValues = new MutablePeriod((ReadableDuration)duration,type,chrono).getValues();
    		else
    			iValues = new MutablePeriod((ReadableInterval)duration,type,chrono).getValues();
    	}
    }
