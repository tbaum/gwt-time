...
import org.gwttime.time.base.BaseLocal;
import org.gwttime.time.chrono.ISOChronology;
import org.gwttime.time.field.AbstractReadableInstantFieldProperty;
...
public final class LocalDateTime
    ...
    private transient Chronology iChronology;
     ...
     */
    public static LocalDateTime fromFields(int year, int month, int dayOfMonth, int hourOfDay, int minute, int second, int millis){
        return new LocalDateTime(year, month,dayOfMonth,hourOfDay,minute,second,millis);
    }
...
//    }
    ...
    public LocalDateTime(ReadablePartial partial){
    	this(partial,(DateTimeZone)null);
    }
    
    public LocalDateTime(ReadablePartial partial, DateTimeZone zone){
    	this(partial, DateTimeUtils.getChronology(partial.getChronology()).withZone(zone));
    }
    
    public LocalDateTime(ReadablePartial partial,Chronology chrono){
    	chrono = DateTimeUtils.getChronology(chrono);
    	iChronology = chrono.withUTC();
    	int [] values = Partials.getCheckedPartialValues(partial, this, chrono);
    	if(values.length!=4)
    		throw new IllegalArgumentException("Cannot create LocalDateTime from the specified ReadablePartial");
    	
    	iLocalMillis = iChronology.getDateTimeMillis(values[0],values[1],values[2],values[3]);
    }
    
    public LocalDateTime(Date date){
    	this(date, (DateTimeZone)null);
    }
    
    public LocalDateTime(Date date, DateTimeZone zone){
    	this(date, ISOChronology.getInstance(zone));
//    	Chronology newChron = ISOChronology.getInstance(zone);
//    	DateTimeZone chronoZone = newChron.getZone();
//    	if(chronoZone!=zone){
//    		newChron = newChron.withZone(zone);
//    		if(newChron==null){
//    			newChron = ISOChronology.getInstance(zone);
//    		}
//    	}
////    	iChronology = DateTimeUtils.getChronology(newChron).withUTC();
//    	iChronology = DateTimeUtils.getChronology(newChron);
//    	long instant = date.getTime();
//    	int [] values = iChronology.get(this,instant);
//    	iLocalMillis = iChronology.getDateTimeMillis(values[0],values[1],values[2],values[3]);
    }
    
    public LocalDateTime(Date date,Chronology chrono){
    	chrono = DateTimeUtils.getChronology(chrono);
    	iChronology = chrono.withUTC();
    	long instant = date.getTime();
    	int [] values =  chrono.get(this,instant);
    	iLocalMillis = iChronology.getDateTimeMillis(values[0], values[1], values[2], values[3]);
    }
...
//    }
	...
	public Date toDate() {
        ...
        if (check.isBefore(this)) {
            ...
            if(dtz.isLocalDateTimeGap(this)){
                Date earlier = new Date(date.getTime()-ONE_HOUR);
    ...
    public String toString() {
        return ISODateTimeFormat.dateTime().print(this.toDateTime());
    }
    ...
    public String toString(String pattern) {
        ...
        return DateTimeFormat.forPattern(pattern).print(this.toDateTime());
    }
    ...
    public String toString(String pattern, LocaleInfo locale) throws IllegalArgumentException {
        ...
        return DateTimeFormat.forPattern(pattern).withLocale(locale).print(this.toDateTime());
    }
    ...
    public static final class Property extends AbstractReadableInstantFieldProperty {
         ...
         */
        public LocalDateTime setCopy(String text, LocaleInfo locale) {
